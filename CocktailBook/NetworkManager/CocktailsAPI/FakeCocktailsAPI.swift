...
class FakeCocktailsAPI: CocktailsAPI {
    ...
    private let jsonData: Cocktails
    ...
    init(withFailure failure: CocktailAPIFailure = .never) {
        ...
        guard let model = try? JSONDecoder().decode(Cocktails.self, from: data) else {
            fatalError("can not load contents of sample.json")
        }
        jsonData = model
    ...
    var cocktailsPublisher: AnyPublisher<Cocktails, CocktailsAPIError> {
        if case let .count(count) = failure {
            failure = count - 1 == 0 ? .never : .count(count - 1)
            return Future<Cocktails, CocktailsAPIError> { [weak self] promise in
                self?.queue.async {
                    sleep(3)
